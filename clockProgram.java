...
public class clockProgram {
	...
	final GpioPinDigitalOutput pin1 = gpio.provisionDigitalOutputPin(RaspiPin.GPIO_01, "pin1", PinState.HIGH); //pin settings
    ...
    final ScheduledExecutorService executorService = Executors.newSingleThreadScheduledExecutor(); //dumb junk that gets the funciton to run once a second
    executorService.scheduleAtFixedRate(new Runnable() {
		@Override
		public void run() {
			
            
            LocalTime time = LocalTime.now();                                          //gets time (in a better way??)
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("HH:mm:ss");     //object that sets the time format
            String formattedTime = time.format(formatter);                             //saves time as string formatted                         
            
            int h1 = Integer.parseInt(Character.toString(formattedTime.charAt(0)));   //parse string into integer digits
            int h2 = Integer.parseInt(Character.toString(formattedTime.charAt(1)));   //uggh so much parsing
            int m1 = Integer.parseInt(Character.toString(formattedTime.charAt(3)));
            int m2 = Integer.parseInt(Character.toString(formattedTime.charAt(4)));
            int s1 = Integer.parseInt(Character.toString(formattedTime.charAt(6)));
            int s2 = Integer.parseInt(Character.toString(formattedTime.charAt(7)));
            
            switch (h1)                         //all of the following junk will set the outputs depending on the digit and its value
            {
                case 0:
